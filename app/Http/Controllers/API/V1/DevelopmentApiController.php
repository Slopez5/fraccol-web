...
class DevelopmentApiController extends Controller
{
    ...
    public function createDevelopment(Request $request)
    {
        ...
        $development->total_lotes = $request['total_lotes'];
        $development->available_lotes = $request['available_lotes'];
    ...
    public function addLoteTypeToDevelopment($developmentId, Request $reuqest)
    {
        ...
        $loteType = LoteType::find($reuqest["lote_type_id"]);
        $development->loteTypes()->attach($loteType, ["price" => $reuqest["price"]]);
    ...
    public function addPaymentPlanToDevelopment($developmentId, $loteTypeId, Request $reuqest)
    {
        ...
        $development->paymentPlans()->attach($paymentPlan, ['lote_type_id' => $loteTypeId, 'price_per_sqm' => $reuqest["price_per_sqm"], 'down_payment' => $reuqest["down_payment"]]);
    ...
    {
        $development = Development::with(['realEstatesAgency', 'realEstatesBranch', 'loteTypes', 'paymentPlans', 'lotes'])->where('id', $id)->get()->first();
        $development->loteTypes->map(function ($loteType) {
    ...
    public function createLote($developmentId, Request $reuqest)
    {
        ...
        if ($reuqest["isMultiLote"] == 1) {
            ...
            $lote->development()->associate($development);
            $lote->loteType()->associate($loteType);
            $lote->lote_number = $reuqest["lote_number"];
            $lote->block_number = $reuqest["block_number"];
            $lote->lote_size = $reuqest["lote_size"];
            $lote->save();
    ...
    {
        $lote = Lote::find($request->query("lote"));
        if ($lote) {
            if ($request->query("payment_plan")) {
                $request->query("payment_plan");
                $paymentSelected = $lote->development->loteTypes->where("id", "=", $lote->lote_type_id)->first()->paymentPlans->where('id', '=', $request->query("payment_plan"))->first();
                ...
                if ($request->query("down_payment")) {
                    ...
                    $total_financing = ($paymentSelected->pivot->price_per_sqm * $lote->lote_size) - $downPayment;
                    $total_price = ($paymentSelected->pivot->price_per_sqm * $lote->lote_size);
                    $lote_size =  $lote->lote_size;
    ...
    {
        $lote_types = Development::find($developmentId)->loteTypes->map(function ($loteType) {
            $loteType->price = $loteType->pivot->price;
            unset($loteType->pivot);
            return $loteType;
        });
    ...
    {
        $payment_plans = Development::find($developmentId)->loteTypes->map(function ($development) {
