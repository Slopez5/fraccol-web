            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-12 col-sm-6">
                        <h3 class="d-inline-block d-sm-none">Fraccionamiento {{ $development->name }}</h3>
                        <div class="col-12">
                            <img src="{{ $images[0]->image_url }}" class="product-image" alt="Product Image">
                        </div>
                        <div class="col-12 product-image-thumbs">
                            <div class="product-image-thumb active">
                                <img src="{{ $images[0]->image_url }}" alt="Product Image">
                            </div>
                    ...
                    <div class="col-12 col-sm-6">
                        <h3 class="my-3">Fraccionamiento {{ $development->name }}</h3>
                        <p style="white-space: pre-line;">{{ $development->sort_description }}</p>
                        <hr>
                        <h4>Financiamientos disponibles</h4>
                        ...
                        <h4 class="mt-3">Lote <small>Selecciona un tipo de lote</small></h4>
                        <div class="btn-group btn-group-toggle" data-toggle="buttons">
                            <label class="btn btn-default text-center">
                              ...
                              <label class="btn btn-default text-center">
                                <input type="radio" name="color_option" id="color_option_b1" autocomplete="off">
                                <span class="text-xl">BC</span>
                                <br>
                                A bordo de carretera
                              </label>
                        </div>

                        <h4 class="mt-3">Lote <small>Selecciona un lote</small></h4>
                        <div class="btn-group btn-group-toggle" data-toggle="buttons">
                            <div class="form-group">
                                <select class="custom-select form-control-border" id="exampleSelectBorder">
                                    <option>Lote 1 Manzana 1 (144m<sup>2</sup>)</option>
                                    <option>Lote 1 Manzana 2 (144m<sup>2</sup>)</option>
                                    <option>Lote 1 Manzana 3 (144m<sup>2</sup>)</option>
                                  </select>
                            </div>
                        </div>
...
@section('extra-scripts')
    <script>
        window.addEventListener('load', function() {
            // Obtener el objeto SVG cargado
            var svgObject = document.getElementById('svgObject').contentDocument;
            console.log(svgObject);
            // Obtener todas las capas dentro del SVG
            var capas = svgObject.querySelectorAll('[id^="Capa_"]');

            // Recorrer todas las capas y agregar eventos de clic
            capas.forEach(function(capa) {
                capa.addEventListener('click', function() {
                    var capaId = capa.id;
                    console.log('Se hizo clic en la capa:', capaId);
                    // Aquí puedes realizar las acciones deseadas para la capa específica
                });
            });
        });
    </script>
